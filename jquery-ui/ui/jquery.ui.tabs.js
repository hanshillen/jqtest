...
$.widget("ui.tabs", {
	...
	_create: function() {
		...
		if (options.label !== "")
		    this.list.attr("title", options.label);
		if (options.labelledBy !== "")
            this.list.attr("aria-labelledby", options.labelledBy);
		...
		this.list.keydown( function(event){
            ...
            switch (event.keyCode) {
                ...
                case $.ui.keyCode.END: 
                    event.preventDefault();
                    selectedIndex = self.anchors.length - 1
                    break;
            ...
            if (selectedIndex !== undefined) {
                selectedIndex = selectedIndex >= self.anchors.length ? 0 : selectedIndex < 0 ? self.anchors.length - 1 : selectedIndex; 
                self.select(selectedIndex);
                self.anchors.eq(selectedIndex).focus();
            }
		}); 
    },
	...
	_tabify: function(init) {
		...
		if (o.event != 'mouseover') {
			...
			//TODO: blur doesn't fire in Firefox when switching tabs, using focusout for now
			this.anchors.bind('blur.tabs focusout.tabs', function() { 
	...
	load: function(index) {
		...
		this.xhr = $.ajax($.extend({}, o.ajaxOptions, {
			...
			success: function(r, s) {
		        ...
		        if($.data(self.anchors[index], 'href.tabs')) {
		            $(self.panels[index])
